...
def extract_glcm_features(image, distances=[1, 3, 5], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4]):
    ...
    print("Step: Resizing image for GLCM.")
    resized = cv2.resize(gray, (128, 128))

    features = []
    print("Step: Calculating GLCM features.")
    for distance in distances:
        for angle in angles:
            glcm = graycomatrix(resized, distances=[distance], angles=[angle], symmetric=True, normed=True)
...
def classify_land_cover(image, glcm_features, n_clusters=3):
    """
    Classify land cover using K-means clustering with GLCM and color features.
    """
    ...
    # Combine GLCM features with color features
    combined_features = np.concatenate([scaled_color_features, glcm_features.T], axis=1)
...
def process_satellite_images(input_folder, output_folder):
    """
    Process satellite images with GLCM feature extraction and advanced segmentation.
    """
    ...
    def process_image(image_path, filename):
        try:
            ...
            print(f"Extracted {glcm_features.shape[0]} GLCM features from {filename}")

            # Perform segmentation with GLCM features integrated
            segmented_image = classify_land_cover(image, glcm_features)

            output_path = os.path.join(output_folder, f'segmented_glcm_{filename}')
...
if __name__ == "__main__":
    ...
    output_folder = "Segmented_GLCM"  # Path to the folder where processed images will be saved
